...
public class QLearningTest implements World{
	...
	class QLearningAction implements Action
	{
		...
		public QLearningAction(String _label)
		{
			m_label = _label;
		}
	...
	public void buildWorldFromMap(Map _map)
	{
		...
		for(int i = 0 ; i<_map.getGridHeight(); ++i)
		{
			for(int j = 0 ; j<_map.getGridWidth() ; ++j)
			{
				...
				case BACK_WALL:
					...
					position = new Point2f(j*UtilsLemmings.s_tileWidth + UtilsLemmings.s_tileWidth / 2.0f, i*UtilsLemmings.s_tileHeight);
	...
	public double computeDistanceToEnd(Point2f _position)
	{
		LevelEnd levelEnd = m_simulation.getEnvironment().getFirstLevelEnd();
		RectangleShape endLevelRect = (RectangleShape)levelEnd.getCollisionMask().getProperty().getActivationZone();
