...
public class QuadTree
{
	...
	public class QuadTreeNode
	{
		...
		private LinkedList<CollisionShape> m_elements = new LinkedList<CollisionShape>();
		
		public QuadTreeNode(RectangleShape _shape)
		{
			m_shape = _shape;
			m_shape.updateShape();
		...
		public boolean addElement(CollisionShape _elem)
		{
			if(_elem.collide(m_shape))
			{
				...
				{
					return m_nw.addElement(_elem) | m_ne.addElement(_elem) | m_sw.addElement(_elem) | m_se.addElement(_elem); 
				}
			}
			else
			{
		...
		public boolean removeElement(CollisionShape _elem)
		{
			if(isLeaf())
			{
			...
			{
				boolean elemRemoved = m_nw.removeElement(_elem) 
						| m_ne.removeElement(_elem)
						| m_sw.removeElement(_elem)
						| m_se.removeElement(_elem);
		...
		{
			m_nw = new QuadTreeNode(new RectangleShape(m_shape.getUpperLeft(), m_shape.getCenter(), null));
			m_ne = new QuadTreeNode(new RectangleShape(m_shape.getUpper(), m_shape.getRight(), null));
			m_sw = new QuadTreeNode(new RectangleShape(m_shape.getLeft(), m_shape.getBottom(), null));
			m_se = new QuadTreeNode(new RectangleShape(m_shape.getCenter(), m_shape.getBottomRight(), null));
		}
		
		public LinkedList<CollisionShape> getUniqueElements(LinkedList<CollisionShape> _out)
		{
			if(_out == null)
	...
	public QuadTree()
	{
		m_rootNode = new QuadTreeNode(new RectangleShape(800, 600, null));
	}
	
	/*----------------------------------------------*/
	public QuadTree(RectangleShape _shape)
	{
		m_rootNode = new QuadTreeNode(_shape);
	}
	
	/*----------------------------------------------*/
	public void add(CollisionShape _shape)
	{
		m_rootNode.addElement(_shape);
	}
	
	/*----------------------------------------------*/
	public void remove(CollisionShape _shape)
	{
	...
	/*----------------------------------------------*/
	public void updateElement(CollisionShape _elem)
	{
	...
	/*----------------------------------------------*/
	public void updateElements(LinkedList<CollisionShape> _elems)
	{
